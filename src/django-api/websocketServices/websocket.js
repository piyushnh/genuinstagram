...
var socket = new WebSocket(`${config.settings.websocketApi}ws/getNotifications/?token=${authToken}`, );
...
const onVisibilityChange = () => {
  ...
  if (!document.hidden && socket.readyState === WebSocket.CLOSED) {
      ...
      socket = new WebSocket(`${config.settings.websocketApi}ws/getNotifications/?token=${authToken}`);
  }
