...
export class HttpService {
  ...
  async getWithoutAuth(url) {
    ...
    const validURL = config.settings.prettyURL
      ? url
      : config.rewrites[url] || url
    const result = await axios.get(`${config.settings.api}${validURL}`)
    return result.data
  }
  ...
  async postWithoutAuth(url, payload) {
    try {
      const result = await axios.post(`${config.settings.api}${url}`, payload)
      return result.data
    } catch (error) {
      console.log(error)
      // let errorData = new SocialError('HttpService/WrongSetting', 'Error happened!')
